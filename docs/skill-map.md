...
| `/weekly-review` | Weekly synthesizer — Snape's read of your week | End of week: what you did, what you avoided, what's next |
...
| `/context-audit` | Memory calibrator — updates the system's model of you | When your situation changes significantly, or when Snape feels stale |
...
Career loop:
  /horizon-scan → /goal-review → /snape → /interview → /cv-job-match → /portfolio-capture
...
- **Have an idea to explore?** → `/sheldon`
- **Want to learn something?** → `/sheldon` routes to `/learn`, or call `/learn` directly
...
- **Career question or feeling stuck?** → `/snape`
